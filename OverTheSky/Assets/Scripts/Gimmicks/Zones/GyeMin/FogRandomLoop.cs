...
{
    [Header("필수: 안개 프리팹(Foz_Zone) 넣기")]
    [SerializeField] private GameObject fogPrefab;

    [SerializeField] private bool runLoopOnStart = false; // 자동 루프(매니저 사용 시 false)
    ...
    {
        if (ps == null) return; // 파티클 준비 전 호출 방지
    ...
    {
        if (loopCo != null) { StopCoroutine(loopCo); loopCo = null; } // 겹침 방지
        if (fogPrefab == null) yield break;

        CreateOrReuseInstance();
        StopAndClear(); // 이전 잔상 제거
    ...
    public void ForceStop()
    {
        ...
        if (ps == null) return; // 아직 파티클이 준비 안 됐으면 종료만
        ApplyIntensity(0f);
        StopAndClear();
    }
